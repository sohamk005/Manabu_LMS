...
const MyEnrollments = () => {
  ...
  fetchUserEnrolledCourses, backendUrl, getToken, calculateNOfLectures} = useContext(AppContext)
  ...
  const getCourseProgress = async () => {
    try{
       ...
       const tempProgressArray = await Promise.all(
        ...
        let totalLectures = calculateNOfLectures(course)
  ...
  return (
    <>
      ...
      <table className='md:table-auto table-fixed w-full overflow border mt-10'>
          ...
          {enrolledCourses.map((course, index) => (
            <tr key={index} className='border-b border-gray-500/20'>
              <td className='md:px-4 pl-2 py-3 flex items-center space-x-3'>
                <img src={course.courseImage} alt='' className='w-14 sm:w-24 md:w-28'/>
